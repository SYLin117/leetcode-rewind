...
// 
//
// Related Topics Two Pointers String 👍 3215 👎 699
...
public class CountBinarySubstrings {
    ...
    class Solution {
        public int countBinarySubstrings(String s) {
            // ans: 答案數, prev: 前面的連續字符數, cur:當前的連續字符數
            int ans = 0, prev = 0, cur = 1;
            for (int i = 1; i < s.length(); i++) {
                if (s.charAt(i - 1) != s.charAt(i)) {
                    ans += Math.min(prev, cur); // 因為prev與cur要相同數量才可以滿足
                    prev = cur;
                    cur = 1;
                } else {
                    cur++;
            ...
            return ans + Math.min(prev, cur);// 最後的組合沒算到
        }
    }
